...
def extract(game, path, destination):
    game_binary = "%s/%s/%s" % (path,game['source_dir'],game['source_binary'])
    ...
    if "%x" % extract_checksum == game['checksum'] :
        if not os.path.exists(destination + '/' + game['destination_dir'] ): os.mkdir(destination + '/' +game['destination_dir'])
        target = destination + '/' + game['destination_dir'] + '/' + game['destination_file']
        r = open(target,"wb")
        ...
        if game['name'] == 'Loom Audio':
          ...
          for byte in data:
            if not (byte[0] % 1177):
              ...
              if (byte[0] % 2 == 0 and frame % 2 == 0) or (byte[0] % 2 == 1 and frame % 2 == 1):
                shiftBy = lshift
              else:
                shiftBy = rshift
            ...
            if frame % 2000 == 0:
              sys.stdout.write("%s %s %s\r" % ("Frame", frame, "of 246227."))
              sys.stdout.flush()
          sys.stdout.write("\n")
          sys.stdout.flush()
          extract = newExtract
...
if __name__ == "__main__":
  ...
  if sys.platform == "darwin" or sys.platform.startswith("linux"):
      ...
      if len(sys.argv) > 1:
        ...
        source = home_dir+"/Library/Application Support/Steam/SteamApps/common"
      else:
      ...
      for game in pc:
        extract(pc[game],shell.SHGetPathFromIDList (pidl),shell.SHGetPathFromIDList (pidl))
